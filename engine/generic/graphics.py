...
class Screen(object):
    ...
    def flip(self):
        ...
        youssef = hasattr(self.config, 'YOUSSEF_FLIP_DELAY')
        if youssef and self.config.YOUSSEF_FLIP_DELAY == 'yes':
        ...
        elif hasattr(self.config, 'INSERT_FLIP_DELAY') and self.config.INSERT_FLIP_DELAY:
           if self.config.ALTERNATIVE_TIMING:
               next_flip_time = self.flip_time_previous + 1.0 / self.config.SCREEN_EXPECTED_FRAME_RATE            
               while True:
                   if next_flip_time <= time.time():
